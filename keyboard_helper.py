...
def on_press(key, csv_filename):
    ...
    try:
        ...
        if key == keyboard.Key.space or key == keyboard.Key.backspace:
            ...
            if current_event:
                write_event_to_csv(current_event,csv_filename)
            current_event = []
        else:
    ...
    write_event_to_csv(['KeyPress', str(key), '', '', 0, elapsed_time],csv_filename)

def on_release(key,csv_filename):
    ...
    # Write the event data to the CSV file
    write_event_to_csv(['KeyRelease', str(key), '', '', 0, elapsed_time],csv_filename)